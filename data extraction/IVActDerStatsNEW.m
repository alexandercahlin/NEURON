function [parameterValuesAct,Ichange,stdIchange] = IVActDerStatsNEW(cellsUndrugged,cellsDrugged)
...
%On Server
IV(1)={importfile('/srv/data/Analysis/LundbeckModelling/IV_recov.xlsx', 'IV','B4:AA26')};
IV(2)={importfile('/srv/data/Analysis/LundbeckModelling/IV_recov.xlsx', 'IV','B28:AA50')};
...
for z=1:2               %loops over both conditions (pre/post drug)
    ...
    for i=0:(TotalC-1)
        ...
        Idens=IV{z}(:,1+4*i);           %selects "leftover" Idens
        tempI(:,i+1)=Idens;             %adjusts indices
        V=IV{z}(:,2+4*i);               %selects voltages
        hold on
    ...
    for i=1:vLevels
        for j=0:(TotalC-1)
            yvaluesIV(i,j+1)=IV{z}(i,1+4*j);    %imports pId(V) values
    ...
    if z==2
        errorbar(xIV,ymeanIV(:,z),2*ySDIV(:,z),'r')
    end
    ...
    for i=0:(TotalC-1)
        V=10^(-3).*IV{z}(:,2+4*i);
        gdens(:,1+i)=IV{z}(:,1+4*i)./(V-vRev); %calcs g(V) per Vstep,cell
        hold on
    ...
    axis([-140 0 -0.2 1.2])
    for i=0:(TotalC-1)
        V=10^(-3).*IV{z}(:,2+4*i);  %retrieves Vsteps per cell
    ...
    for i=0:(TotalC-1)
        if ismember(i,incl)==1
            V=10^(-3).*IV{z}(:,2+4*i); %scales V to mV
            vLev=sum(~isnan(V));
            %perform fit, extract gof values
            [sigmoid, gof] = ActSigmoidfitOWN(V(1:vLev)*10^3, gdens(1:vLev,i+1));
    ...
    for i=0:(TotalC-1)  %retrieves fits, plots the sigmoids per cell
        y=(1./(1+exp((x-ActV(i+1))*(-1/ActK(i+1))))).^3;
        hold on
        plot(x,y)
    end
