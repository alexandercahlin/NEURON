...
Taus=[tau1,tau2,vStore,wStore,FRecovR2];          %Stores all vectors in a matrix
...
FRecovMeanTau2=nanmean(Taus(:,2));  %Calculates the mean of Tau2
FRecovSDTau2=nanstd(Taus(:,2));     %Calculates the SD of Tau2  

vStoremean=nanmean(Taus(:,3));      %Calculates the mean of v
wStoremean=nanmean(Taus(:,4));      %Calculates the mean of w
...
tau1=FRecovMeanTau1;
tau2=FRecovMeanTau2;
meanCurveTau=vTau.*(1-exp(-hTau/tau1))+wTau.*(1-exp(-hTau/tau2));
...
for i=0:(TotalC-1)
        ...
        [biexp, gof] = FastTauFit(x, y);
        tau1Drug(i+1)=biexp.tau1;
        tau2Drug(i+1)=biexp.tau2;
        vStoreDrug(i+1)=biexp.v;
        wStoreDrug(i+1)=biexp.w;
